...
//extern  u32       TimeDisplay;                   //RTC定时标志变量
extern  u8        WWDG_CNT;                      //保存WWDG计数器的设置值,默认为最大
extern  u16       WWDOG_Feed ;                   //窗口看门狗复位周期为：255*1.8s = 7.6min
extern  u8        Uart4_send_buff[100]; 
extern  u8        Uart4_rev_buff[100];
//extern  vs8       Uart4_send_counter;
extern  u8        Uart4_rev_count;               //RS485串口接收计数器
extern  u8        Uart4_rev_comflag;             //RS485串口接收完成标志变量
...
//		Usart1_recev_buff[Usart1_recev_count++] = USART_ReceiveData(USART1);
			USART_ReceiveData(USART1);                                        //放弃数据接收
...
//	  printf("\r\nEnter USART2_IRQ!! \r\n");     //测试使用
    ...
    {  
        ...
        USART_ClearITPendingBit(USART2, USART_IT_IDLE);
			  ...
			  i =RECEIVEBUFF_SIZE - DMA_GetCurrDataCounter(DMA1_Channel6);  //测试使用
//			  printf("\r\nDMA Recev Length:%d\r\n",i);                      //测试使用
...
////中断,检测到PA0脚的一个上升沿.	  
////中断线0线上的中断检测
//void EXTI0_IRQHandler(void)                    //测试使用
//{ 		    		    				     		    
